...
# AI Motorumuzu ve Trip şemamızı içeri aktarıyoruz
...
async def calculate_route(
    origin: str, 
    destination: str, 
    db: Session = Depends(get_db), # YENİ EKLENDİ: Veritabanı bağlantısı lazım
    current_user: User = Depends(get_current_user)
):
    """
    Google Maps rotasını hesaplar ve bu rota üzerindeki mekanları (POIs) bulur.
    """
    # 1. Google'dan rotayı ve Polyline şifresini al
    route_data = await get_directions(origin=origin, destination=destination)
    
    # 2. Şifreli polyline'ı veritabanına gönderip rota üzerindeki (15km) mekanları bul
    pois_on_route = get_pois_along_route(
        db=db, 
        polyline=route_data["polyline"], 
        radius_km=15 # 15 km sağa ve sola sapma payı
    )
    
    # 3. Hepsini birleştirip tek bir zeka paketi olarak dışarı gönder
    return {
        "route_info": route_data,
        "recommended_pois": pois_on_route
    }
...
async def generate_ai_route_plan(
    ...
    route_data = await get_directions(origin=trip_data.start_location, destination=trip_data.destination_location)
    
    # 2. Mekanları bul
    pois_on_route = get_pois_along_route(db=db, polyline=route_data["polyline"], radius_km=15)
    
    # 3. Context ve Details
    user_context = {
    ...
    # 4. AI'dan Planı Al
    ai_plan = generate_travel_plan(user_context, trip_details_dict, route_data, pois_on_route)
    
    # 5. YENİ EKLENEN: Veritabanına Kaydetme İşlemi!
    new_trip = Trip(
        ...
        # AI'dan gelen verileri veritabanı sütunlarına yazıyoruz:
        route_polyline=route_data["polyline"],
        ai_itinerary=ai_plan.model_dump() # Pydantic objesini JSON sözlüğüne çeviriyoruz
    )
    ...
    # 6. Mobil uygulamaya ID ile birlikte geri dönüyoruz
    return {
        "trip_id": new_trip.id,
